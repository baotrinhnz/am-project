...
export default function MusicBpmWidget({ range, deviceId }) {
  ...
  useEffect(() => {
    ...
    const sub = supabase
      .channel('music_bpm_readings_channel')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'music_bpm_readings' }, payload => {
      ...
      <div className="mb-4">
        {loading ? (
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-4xl font-mono font-bold" style={{ color: '#a78bfa' }}>
              {Math.round(latest.bpm)}
            </span>
            <span className="text-sm" style={{ color: 'var(--text-tertiary)' }}>BPM</span>
          </div>
        ) : (
          <div className="text-sm" style={{ color: 'var(--text-tertiary)' }}>No music detected</div>
        )}
      </div>
      ...
      {data.length > 1 && (
        <ResponsiveContainer width="100%" height={80}>
          <AreaChart data={data} margin={{ top: 0, right: 0, left: -30, bottom: 0 }}>
            <defs>
              <linearGradient id="musicBpmGrad" x1="0" y1="0" x2="0" y2="1">
            ...
            <Tooltip
              ...
              formatter={(v) => [`${Math.round(v)} BPM`, 'Music Beat']}
            />
            <Area type="monotone" dataKey="bpm" stroke="#a78bfa" strokeWidth={1.5}
              fill="url(#musicBpmGrad)" dot={false} />
          </AreaChart>
        </ResponsiveContainer>
      )}
    </div>
